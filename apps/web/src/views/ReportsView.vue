...
async function load() {
  ...
  try {
    overview.value = await api<Overview>(
      `/reports/overview?from=${from.value}T00:00:00.000Z&to=${to.value}T23:59:59.999Z`,
    )
  } catch (e) {
...
async function downloadCsv() {
  try {
    ...
    const base =
      ...
      `${API_ORIGIN}/api/v1`
    const url = `${base}/reports/revenue.csv?from=${from.value}T00:00:00.000Z&to=${to.value}T23:59:59.999Z`
  ...
  <div class="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
    <header class="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-ink-muted">Ingresos, ocupación y no-shows del periodo.</p>
      </div>
      <div class="flex flex-wrap items-end gap-2">
        <label class="text-xs text-ink-muted">
          Desde
          <input v-model="from" type="date" class="mt-1 block rounded-xl border border-black/10 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-ink-soft" />
        </label>
        <label class="text-xs text-ink-muted">
          Hasta
          <input v-model="to" type="date" class="mt-1 block rounded-xl border border-black/10 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-ink-soft" />
        </label>
    ...
    <p v-if="error" class="rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-800 dark:bg-red-950/40 dark:text-red-200">
      {{ error }}
    </p>
    ...
    <template v-else-if="overview">
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <article class="surface p-4">
          ...
          <p class="text-xs text-ink-muted">{{ overview.paymentsCount }} pagos</p>
        </article>
        <article class="surface p-4">
        ...
        <article class="surface p-4">
          ...
          <p class="text-xs text-ink-muted flex items-center gap-1"><BarChart3 class="h-3.5 w-3.5" /> periodo</p>
        </article>
      </div>
